...
int main()
{
    ...
    if (listen_sock_tcp < 0)
        error_exit("TCP socket failed", LOG_ERR);
    ...
    if (discovery_sock_udp < 0)
        error_exit("UDP discovery socket failed", LOG_ERR);
    ...
    {                                          // Pętla działa, dopóki flaga terminate_server nie zostanie ustawiona (np. przez SIGINT)
        ...
        {
            if (errno == EINTR)
            {             // select() przerwany przez sygnał (np. SIGCHLD)
                continue; // Ponów pętlę
            }
            error_exit("Select error", LOG_ERR); // Inny błąd select()
        }
        if (terminate_server)
            break; // Sprawdź flagę po select()
        ...
        if (FD_ISSET(listen_sock_tcp, &read_fds))
        {
            ...
            if (new_sock < 0)
            {
                if (errno == EINTR && terminate_server)
                    break; // Obsługa przerwania accept przez sygnał
